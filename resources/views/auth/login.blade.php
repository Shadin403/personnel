...
<body class="h-full flex items-center justify-center p-6 bg-slate-950 overflow-hidden font-sans">
    ...
    <div class="relative w-full max-w-[440px] z-10 animate-fade-in">
        ...
        <div class="text-center mb-12">
            ...
            <h1 class="text-2xl font-bold text-white tracking-[.25em] uppercase">
                CORE <span class="text-military-accent">UPLINK</span>
            </h1>
            <div class="mt-2 flex items-center justify-center gap-3">
                <div class="h-[1px] w-8 bg-military-accent/40"></div>
                <p class="text-slate-500 font-bold text-[10px] uppercase tracking-[.4em]">Strategic Control Node</p>
                <div class="h-[1px] w-8 bg-military-accent/40"></div>
            </div>
        </div>
        ...
        <div class="classic-card rounded-none p-10 bg-military-bg relative">
            <!-- Security Identifier -->
            <div class="absolute top-0 right-0 px-4 py-1 bg-military-primary text-white text-[8px] font-bold uppercase tracking-widest">
                VER-A.02
            </div>
            ...
            <form action="{{ route('login.post') }}" method="POST" class="space-y-8">
                ...
                <div class="space-y-2">
                    <label for="email" class="text-[9px] font-bold text-military-secondary ml-1 uppercase tracking-[0.2em]">Personnel Email (AUTH_ID)</label>
                    <div class="relative group">
                        ...
                        <input type="email" id="email" name="email" value="{{ old('email', 'admin@gmail.com') }}" required
                               class="w-full pl-12 pr-4 py-4 bg-white border @error('email') border-military-danger @else border-slate-300 @enderror rounded-none text-slate-900 text-xs font-semibold uppercase tracking-wider focus:outline-none focus:ring-1 focus:ring-military-primary focus:border-military-primary transition-all shadow-inner"
                               placeholder="ADMIN@COMMAND.GOV">
                    </div>
                    @error('email')
                ...
                <div class="space-y-2">
                    <label for="password" class="text-[9px] font-bold text-military-secondary ml-1 uppercase tracking-[0.2em]">Access Key (CIPHER)</label>
                    <div class="relative group">
                        <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-military-secondary/40 group-focus-within:text-military-primary transition-colors">
                        ...
                        <input type="password" id="password" name="password" required
                               class="w-full pl-12 pr-4 py-4 bg-white border @error('password') border-military-danger @else border-slate-300 @enderror rounded-none text-slate-900 text-xs font-semibold uppercase tracking-wider focus:outline-none focus:ring-1 focus:ring-military-primary focus:border-military-primary transition-all shadow-inner"
                               placeholder="••••••••">
                    </div>
                    @error('password')
                        <p class="text-military-danger text-[9px] font-bold uppercase tracking-tight mt-1 ml-1 animate-shake">{{ $message }}</p>
                ...
                <div class="flex items-center justify-between">
                    <label class="flex items-center cursor-pointer group">
                        ...
                        <span class="ml-3 text-[10px] font-bold text-slate-500 group-hover:text-military-secondary transition-colors uppercase tracking-widest">Persist Session Node</span>
                    </label>
                </div>

                <button type="submit" 
                        class="btn-military w-full py-4 text-[11px] shadow-lg active:scale-95">
        ...
        <div class="mt-10 flex items-center justify-between border-t border-slate-800 pt-6">
            <div class="flex items-center gap-3">
                <div class="w-2.5 h-2.5 bg-military-success animate-pulse shadow-[0_0_8px_#15803D]"></div>
                <span class="text-[9px] font-bold text-slate-500 uppercase tracking-widest">Signal: Optimal</span>
            </div>
            <div class="flex items-center gap-3">
                ...
                <span class="text-[9px] font-bold text-slate-700 uppercase tracking-widest font-mono">UPLINK_SECURE_256</span>
            </div>
        </div>
    </div>
