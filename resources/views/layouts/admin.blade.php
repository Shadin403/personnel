...
<head>
    ...
    <style>
        ...
        ::-webkit-scrollbar-thumb { background: #2F4F3E; border-radius: 0; }
...
<body class="h-full overflow-hidden bg-parchment font-sans text-military antialiased" x-data="{ sidebarOpen: false }">
    <div class="flex h-full">
        <!-- Sidebar -->
        <aside class="fixed inset-y-0 left-0 z-50 w-64 bg-military-secondary text-white transition-transform duration-300 transform lg:translate-x-0 lg:static lg:inset-0 border-r border-black/20 shadow-2xl"
        ...
        <main class="flex-1 flex flex-col h-full bg-parchment overflow-hidden relative">
            <!-- Top Navbar -->
            <header class="flex items-center justify-between h-14 px-6 bg-white border-b border-slate-300 lg:px-8 shadow-sm">
            ...
            <div class="flex-1 overflow-y-auto p-6 lg:p-10 custom-scrollbar">
                <!-- Flash Messages -->
                @if(session('success'))
                    <div class="mb-8 p-4 bg-emerald-50 border-l-4 border-emerald-600 text-emerald-800 flex items-center gap-4 animate-fade-in shadow-sm" x-data="{ show: true }" x-show="show">
                        ...
                        <span class="flex-1 text-[11px] font-bold uppercase tracking-widest">{{ session('success') }}</span>
