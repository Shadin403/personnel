...
@section('content')
    <div class="space-y-8 animate-fade-in pb-20" x-data="{
            ...
            <div class="fixed inset-0 z-[999] flex items-center justify-center bg-military-primary/80 backdrop-blur-md">
                <div class="text-center space-y-6">
                    <div class="relative w-24 h-24 mx-auto">
                        ...
                        <div
                            class="absolute inset-0 border-4 border-military-accent rounded-full animate-spin border-t-transparent">
                        </div>
                    </div>
                    <div class="space-y-2">
                        <h3 class="text-xl font-black text-white uppercase tracking-[0.3em]">Processing Records</h3>
                        <p class="text-[10px] font-bold text-military-accent uppercase tracking-widest animate-pulse">
                            Generating Secure PDF Artifacts...</p>
                    </div>
                </div>
            </div>
        </template>
                ...
                <div class="px-8 py-5 bg-slate-50 flex items-center gap-6">
                    <label class="flex items-center gap-3 cursor-pointer group">
                        <input type="checkbox" @click="toggleAll()"
                            :checked="selectedIds.length === allIds.length && allIds.length > 0" class="tactical-checkbox">
                        <span
                            class="text-[11px] font-black uppercase tracking-widest text-slate-600 group-hover:text-red-600 transition-colors">Select
                            Page Personnel</span>
                    </label>
        ...
        @else
            <!-- Premium Empty State -->
            <div
                class="py-32 classic-card bg-emerald-50/30 border-2 border-dashed border-emerald-200 dark:border-emerald-800 text-center space-y-6">
                <div
                    class="w-20 h-20 bg-emerald-100 dark:bg-emerald-900/40 rounded-full flex items-center justify-center mx-auto shadow-inner">
                ...
                <div class="max-w-md mx-auto">
                    <h3 class="text-2xl font-black text-slate-900 dark:text-white uppercase tracking-tight">Operational
                        Excellence</h3>
                    <p class="text-[12px] font-bold text-slate-500 uppercase tracking-[0.2em] mt-3 leading-relaxed"> No
                        personnel currently identified under remediation criteria. All units report status "NORMAL".</p>
                </div>
                <div class="flex justify-center pt-4">
                    <span
                        class="px-4 py-2 bg-emerald-600 text-white text-[10px] font-black uppercase tracking-widest shadow-lg">Verified
                        Operational Registry</span>
                </div>
            </div>
        @endif
        ...
        <div x-show="selectedIds.length > 0" x-transition:enter="transition ease-out duration-300"
            ...
            <div
                class="bg-slate-900 dark:bg-slate-800 text-white p-5 rounded-none shadow-2xl border border-white/10 flex items-center justify-between backdrop-blur-xl ring-1 ring-white/20">
                <div class="flex items-center gap-5">
                    <div
                        class="w-12 h-12 bg-red-600 rounded-none flex items-center justify-center font-black animate-pulse shadow-lg border border-red-400/20">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <button @click="selectedIds = []"
                        class="p-3 hover:bg-red-600 transition-colors bg-white/5 border border-white/10">
