...
<div class="max-w-4xl mx-auto animate-fade-in">
    <div class="mb-8">
        ...
        <h2 class="text-2xl font-black text-military-primary tracking-tight uppercase">Refine Organizational Unit</h2>
        <p class="text-[11px] font-bold text-military-accent uppercase tracking-widest mt-1 opacity-70">
            Node #{{ $unit->id }} - Current Designation: {{ $unit->name }}
        </p>
    </div>
    ...
    <form action="{{ route('admin.units.update', $unit) }}" method="POST" class="space-y-6">
        ...
        <div class="classic-card">
            <div class="classic-card-header">
                <span class="text-[11px] font-bold uppercase tracking-widest">Unit Refinement Parameters</span>
            </div>
            <div class="p-8 space-y-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-2">
                        ...
                        <select name="type" required
                            ...
                            <option value="company" {{ old('type', $unit->type) == 'company' ? 'selected' : '' }}>Company (Coy)</option>
                            <option value="platoon" {{ old('type', $unit->type) == 'platoon' ? 'selected' : '' }}>Platoon (PL)</option>
                            <option value="section" {{ old('type', $unit->type) == 'section' ? 'selected' : '' }}>Section (SEC)</option>
                            <option value="battalion" {{ old('type', $unit->type) == 'battalion' ? 'selected' : '' }}>Battalion</option>
                        </select>
                    ...
                    <div class="space-y-2 md:col-span-2">
                        ...
                        <textarea id="appointment_editor" name="appointment" class="w-full">{{ old('appointment', $unit->appointment) }}</textarea>
                ...
                <div class="flex items-center gap-3 p-4 bg-slate-50 border border-slate-200">
                    <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $unit->is_active) ? 'checked' : '' }}
